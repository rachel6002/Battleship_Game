...
public abstract class Ship {
	...
	/**
	 * {@literal true} if the ship occupies a single row, {@literal false}
	 * otherwise.
	 */
	 ...
	 * the extents of the 2D array in which it is placed. <b>Calling this method
	 * should not actually change either the Ship or the provided Ocean.</b>
	 *
		...
		if (horizontal) {
			...
			for (int i = row - 1; i <= row + 1; i++) {
				for (int j = column - 1; j <= column + getLength(); j++) {
					if (i < 0 || i >= ocean.getShipArray().length || j < 0 || j >= ocean.getShipArray()[0].length) {
						continue;
					}
	...
	public boolean shootAt(int row, int column) {
		if (!isSunk()) {
			if (isHorizontal()) {
				if (row == getBowRow() && column < getBowColumn() + getLength() && column >= getBowColumn()) {
					hit[column - getBowColumn()] = true;
					return true;
				}
			} else {
				if (column == getBowColumn() && row < getBowRow() + getLength() && row >= getBowRow()) {
